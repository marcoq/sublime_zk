        ...
        if window.project_file_name():
            ...
            if not folder:
                ...
                window.status_message(
                    'Zettelkasten mode cannot be entered '
                    'without a project or an open folder!')
                return False
...
PANE_FOR_OPENING_NOTES = 0
PANE_FOR_OPENING_RESULTS = 1
DISTRACTION_FREE_MODE_ACTIVE = collections.defaultdict(bool)
...
class ImageHandler:
    ...
    '''
    Phantoms = collections.defaultdict(set)
...
class Autobib:
    ...
    def remove_latex_commands(s):
        ...
        any LaTeX commands or brackets from the string,
    ...
    def create_bibliography(text, bibfile, pandoc='pandoc'):
        ...
        for all citations in text in form of a dictionary.
        """
...
class ExternalSearch:
    ...
    def search_all_tags(folder, extension):
        ...
        if ExternalSearch.EXTERNALIZE:
            ...
            with open(ef, mode='w', encoding='utf-8') as f:
                f.write('# All Tags\n\n')
    ...
    def notes_and_tags_in(folder, extension):
        ...
        args.extend([
            '--nonumbers', '-o', '--silent', '-G', '.*\\' + extension,
        ...
        lines = collections.deque(ag_out.split('\n'))
    ...
    def search_friend_notes(folder, extension, note_id):
        ...
        prefix = 'Notes referencing {}{}{}:'.format(
            link_prefix,
            note_id,
            link_postfix)
        ExternalSearch.externalize_note_links(
    ...
    @staticmethod
    def externalize_note_links(ag_out, folder, extensions, prefix=None):
        """
        ...
        if ExternalSearch.EXTERNALIZE:
            ...
            with open(ef, mode='w', encoding='utf-8') as f:
                if prefix:
                    f.write(u'{}\n\n'.format(prefix))
                results = []
                for line_ in sorted(ag_out.split('\n')):
                    if not line_.strip():
                        continue
                    for extension in extensions:
                        if line_.endswith(extension):
                            line = os.path.basename(line_)
                ...
                if sort_order == 'title':
                    column = 1
                results.sort(key=operator.itemgetter(column))
                for note_id, title, line in results:
                        f.write(u'* {}{}{} {}\t\t\t\t\t\t\t\t"{}"\n'.format(
                            link_prefix,
                            note_id,
                            link_postfix,
                            title,
                            os.path.join(folder, line)))

    @staticmethod
    def externalize_note_tree(ag_out, folder, extensions, prefix=None):
        """
        If enabled, write ag file name output into external search results file
        in `[[note_id]] note title` style.
        """
        if ExternalSearch.EXTERNALIZE:
            ef = ExternalSearch.external_file(folder)
            with open(ef, mode='w', encoding='utf-8') as f:
                f.write(ag_out.replace('.md', '').replace('\\n', ''))
...
class TextProduction:
    ...
    def embed_note(note_id, folder, extension, link_prefix, link_postfix):
        ...
        """
        result_lines = []
        note_file, content = TextProduction.read_full_note(
            note_id,
            folder,
            extension)
    ...
    def expand_link_in(view, edit, folder, extension):
        ...
        if link_region and not link_is_citekey:
            ...
            note_list = ExternalSearch.search_tagged_notes(
                folder, extension,
...
def create_note(
        filn, title, origin_id=None, origin_title=None, body=None,
        template_setting_key='new_note_template'):
    ...
    params = {
        ...
        'origin': origin_id, }
    settings = get_settings()
    format_str = settings.get(template_setting_key)
    if not format_str:
        format_str = u'# {title}\ntags = \n\n'
    else:
...
def get_path_for(view):
    ...
    if folder is None:
        ...
        view.window().status_message(
            'Could not find the location of your note '
...
def select_link_in(view, event=None):
    ...
    linestart_till_cursor_str = view.substr(
        sublime.Region(
            line_start,
            cursor_pos))
    full_line = view.substr(line_region)
    ...
    if p_symbol_pos >= 0:
        ...
        if note_id:
            ...
            return (linestart_till_cursor_str, sublime.Region(
                p_link_start,
                p_link_end))
    ...
    if brackets_start >= 0:
        ...
        if brackets_end >= 0:
            link_region = sublime.Region(
                line_start + brackets_start +
...
def get_note_id_of_file(filn):
    ...
    """
    settings = get_settings()
    for extension in settings.get('wiki_extensions'):
...
def post_open_note(view, pane):
    ...
    if pane > -1:
        view.window().set_view_index(
            view, pane, 0)   # ,0..make it first view
    else:
        window = view.window()
        ...
        new_cell = [
            old_cell[XMAX], old_cell[YMIN],
            old_cell[XMAX]+1, old_cell[YMAX]]
        old_cell = [
            old_cell[XMIN], old_cell[YMIN],
            old_cell[XMAX], old_cell[YMAX]]
...
class ZkExpandOverviewNoteCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        result_text = TextProduction.expand_links(
            complete_text, folder,
...
class ZkRefreshExpandedNoteCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        result_text = TextProduction.refresh_result(
            complete_text, folder,
            extension)
    ...
    def select_link(self, event=None):
        ...
        * else present overlay to pick a note
        ...
        if link_region:
            ...
            link_is_citekey = (
                link_text.startswith('@') or
                link_text.startswith('#'))
            if not link_is_citekey:
                return link_region

        # test if we are supposed to follow a tag
        if ((ZkConstants.TAG_PREFIX in linestart_till_cursor_str) or
                ('@' in linestart_till_cursor_str)):
            view = self.view
            if event is None:
            ...
            if F_EXT_SEARCH:
                ...
                if ExternalSearch.EXTERNALIZE:
                    n = self.view.window().open_file(
                        ExternalSearch.external_file(
                            folder))
    ...
    def run(self, edit, event=None):
        ...
        * searching for #tagged notes
        ...
        if self.view.match_selector(cursor_pos, 'markup.zettel.search'):
            ...
            #
            note_ids = TagSearch.advanced_tag_search(
                input_text, self.folder,
                self.extension)
            ...
            for note_id in [n for n in note_ids if n]:  # Strip the None
                filn = note_file_by_id(
                    note_id, self.folder, self.extension)
                if filn:
            ...
            if ExternalSearch.EXTERNALIZE:
                with open(
                        ExternalSearch.external_file(self.folder),
                        mode='w',
                        encoding='utf-8') as f:
                    f.write('\n'.join(lines))
            ExternalSearch.show_search_results(
                self.window, self.folder,
...
class ZkNewZettelCommand(sublime_plugin.WindowCommand):
    ...
    def on_done(self, input_text):
        ...
        # sanity check: do we have an open folder
            ...
            folder = os.path.dirname(self.window.project_file_name())
            if not folder:
                # I don't know how to save_as
                # the file so there's nothing sane I
                # can do here. Non-obtrusively warn the user that this failed
                self.window.status_message(
        ...
        if self.insert_link:
            ...
            view = self.window.active_view()
            view.run_command(
                'zk_replace_selected_text', {'args': {'text': link_txt}})
        create_note(
            the_file, new_title,
            self.origin, self.o_title, self.note_body,
            template_setting_key='new_zettel_template')
        new_view = self.window.open_file(the_file)
        post_open_note(new_view, PANE_FOR_OPENING_NOTES)
...
class ZkGetWikiLinkCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        self.modified_files = [
            os.path.basename(f).replace(extension, '')
            for f in self.files]
...
class ZkTagSelectorCommand(sublime_plugin.TextCommand):
    ...
    def on_done(self, selection):
        if selection == -1:
            self.view.run_command(
                'zk_insert_wiki_link',
                {'args': {'text': ZkConstants.TAG_PREFIX}})   # re-used
            return
...
class ZkShowAllTagsCommand(sublime_plugin.WindowCommand):
    ...
    def run(self):
        ...
        if ExternalSearch.EXTERNALIZE and not F_EXT_SEARCH:
            with open(
                ExternalSearch.external_file(folder),
                mode='w',
                encoding='utf-8') as f:
                    f.write(lines)
        ExternalSearch.show_search_results(
            self.window, folder, 'Tags', lines,
            'show_all_tags_in_new_pane')


def get_all_notes_in_dir(folder, extensions):
    note_files = list()
    for extension in extensions:
...
def run_show_all_notes_in_dir(theself, folder, extensions, maxn=None):
        # # sanity check: do we have a project
        # if self.window.project_file_name():
        #     # yes we have a project!
        #     folder = os.path.dirname(self.window.project_file_name())
        ...
        ExternalSearch.externalize_note_links(
            ...
            folder,
            extensions,
            prefix='# All %d Notes:' % len(note_files))
        ef = ExternalSearch.external_file(folder)
        lines = open(
            ef, mode='r', encoding='utf-8', errors='ignore').read(
                ).split('\n')
        ExternalSearch.show_search_results(
            theself.window, folder, 'Notes', lines,
            'show_all_tags_in_new_pane')
...
class ZkShowAllNotesCommand(sublime_plugin.WindowCommand):
    ...
    def run(self):
        ...
        folder = os.path.expanduser(settings.get('notes_base_dir'))
        extensions = settings.get('wiki_extensions')
        return run_show_all_notes_in_dir(self, folder, extensions)
...
class ZkTocCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        ref_counter = collections.Counter(
            {'': 1})   # '' for unprintable char only headings
...
class ZkSelectPanesCommand(sublime_plugin.WindowCommand):
    ...
    def run(self):
        ...
        for group in range(self.window.num_groups()):
            ...
            group_view.add_phantom(
                'popup', sublime.Region(0,0), show_str,
                sublime.LAYOUT_BLOCK)
        self.window.show_input_panel(
            'Pane number for opening NOTES:',
    ...
    def on_done_first(self, text):
        ...
        if F_EXT_SEARCH:
            self.window.show_input_panel(
                'Pane number for opening RESULTS:',
...
class NoteLinkHighlighter(sublime_plugin.EventListener):
    ...
    * Enables word completion (ctrl + space) to insert links to notes.
    ...
    def highlight_note_links(self, view, note_links, tag_regions):
        """
        Creates a set of regions
        from the intersection of note_links and scopes,
        ...
        for scope_name in scope_map:
            self.underline_regions(
                view, scope_name, scope_map[scope_name],
                show_bookmarks,
                tags=False)
...
class ZkShowAllZettelnCommand(sublime_plugin.WindowCommand):
    """
    Command that creates a new view containing a sorted list of all notes
    """
    def run(self):
...
class ZkShowFourRandomNotesCommand(sublime_plugin.WindowCommand):
    ...
    #         self.folder, self.friend_note_files[selection])
    ...
    def run(self):
        ...
        folder = os.path.expanduser(settings.get('notes_base_dir'))
        extensions = settings.get('wiki_extensions')
        return run_show_all_notes_in_dir(self, folder, extensions, maxn=4)
...
class ZkNewNoteCommand(sublime_plugin.WindowCommand):
    ...
    def on_done(self, input_text):
        ...
        if self.insert_link:
            ...
            view = self.window.active_view()
            view.run_command(
                'zk_replace_selected_text',
                {'args': {'text': link_txt}})
        create_note(
            the_file, new_title, self.origin,
...
class ZkShowMimimmiCommand(sublime_plugin.WindowCommand):
    """
    Command that creates a new view containing a sorted list of all notes
    """
    def run(self):
...
class ZkShowZettelnTreeCommand(sublime_plugin.WindowCommand):
    """
